...
const Order = () => {
    ...
    useEffect(() => {
        axios.post(`${config}api/customersPlans`, { Uid: Uid }).then((response) => {
            console.log("response", response)

            //if (response.data.success === "true") {
               // if(response.data.data[0].plan==0){
                    setOrder(response.data.data);
                    setplancount(response.data.data[0].plan);
               // }else{
                //    setplancount(response.data.data[0].plan);
                //}
    ...
    const deleterecord = (ID) => {
        axios.post(`${config}api/deletecustomerPlan`, {ID:ID}).then((response) => {
            if(response.data.success === "true"){
              Swal.fire({
                toast: true,
                icon: 'success',
                    ...
                    <div className="my-order-table-area">
                     <div className="row"> 
                      <div className="col-lg-3">   
                        ...
                        <div>
                            {
                                Orders.forEach(element => <p>{element}</p>)
                            }
                        </div>
