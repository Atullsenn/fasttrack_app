...
const PackagePlanCustomer = () => {
    ...
    const addPlanSubmit = async(event) => {
       ...
       // console.log("tesing else")
        ...
        await axios({
            method: "POST",
            url: `${config}api/updatecustomerPlan`,
            data: json,
            headers: {
                'Content-Type': 'application/json'
              },
    ...
    useEffect(()=>{
        axios.post(`${config}api/singlecustomerPlan`, {ID:{ id }}).then((response) => {
            console.log(response.data)

            if (response.data.success === "true") {
              // console.log(response.data.res)
               setSingleCustomerPlan(response.data.data[0]);
              setradioplan(response.data.data[0].ON_DEMAND);
            }
        });
    },[]);
    const Ondemond =(e)=>{
                                ...
                                <div className="col-lg-4">
                                    <div className="customer-type-option-area">
                                        <div className="form-group">
                                            ...
                                            <label htmlFor="html"><input type="radio" value={"1"} onChange={Ondemond} checked={radioplan=="1"}   name="ondemand"   />On Demand</label>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-lg-4">
                                    <div className="customer-type-option-area">
                                        <div className="form-group">
                                            <label htmlFor="css"><input onChange={Ondemond} value={"0"} checked={radioplan=="0"} type="radio"  name="ondemand"   />Packages</label>
                                        </div>
                                    </div>
                                </div>
