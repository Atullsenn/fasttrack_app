...
const Companies = () => {
  ...
  useEffect(()=>{
    axios.post(`${config}api/companiesList`, {}).then((response) => {
...
function deleterecord(ID){
    ...
    axios.post(`${config}api/deleteCompany`, {ID:ID}).then((response) => {
      if(response.data.success === "true"){
        Swal.fire({
          toast: true,
          icon: 'success',
    ...
    <div className="manage-admins-main-area">
      ...
      <Link type="button" to="/AddEdit" className="btn btn-info rounded-circle ml-auto"><i className="fa fa-plus" aria-hidden="true"></i></Link>
        ...
        <table className="table" id="example2">
          ...
          {Company.length === 0 ? <CircularProgress style={{position:'relative', left:'550px', width:'30px', height:'30px'}}/> : ''}
          <tbody>
          {
