...
const Staffbycompany = () => {
  ...
  useEffect(() => {
    // alert(Uid)
    axios.post(`${config}api/staffByCompany`, { Uid: Uid }).then((response) => {
      console.log(Uid)
...
const deleterecord=(ID)=>{
  axios.post(`${config}api/deleteStaff`, {ID:ID}).then((response) => {
          ...
          <div className="manage-admins-main-area">
            <div className="manage-admins-table-area">
              ...
              <table className="table" id="example2">
                ...
                {
                staff.length === 0 ? <CircularProgress style={{position:'relative', left:'550px', width:'30px', height:'30px'}}/> : ''
               }
                <tbody>
                  {
                    staff.map((resdata, key) => {
                      return (
                        <tr>
                          ...
                          <td>
                          {
                          resdata.role.map((resval, key) => {
                            return ( <p> {resval.CROL_NAME} </p> )
                             })
                         }   
                         </td>
                              <td>
