...
const AddEdit = () => {
    let { id } = useParams();
    const companyimagepath=config+"public/upload/Company/";
    const bannercompanyimagepath=config+"public/upload/ComBanner/";
    ...
    useEffect(() => {
        axios.post(`${config}api/Tlanguage`, {}).then((response) => {
           // console.log(response.data)
            if (response.data.success === "true") {
                setTlanguage(response.data.data);
            }
        })
    }, []);
    const addSubmit = async (event) => {
        ...
        if(event.target.ACC_ID==null){
           ...
           // console.log(bodyFormData)
            ...
            var json = qs.stringify(object);
            //console.log(json)
        await axios({
            method: "post",
            url: `${config}api/addCompany`,
            data: json,
            headers: {
            ...
            await axios({
                method: "post",
                url: `${config}api/UpdateCompaniesList`,
                data: json,
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                  },
    ...
    useEffect(() => {
        axios.post(`${config}api/singleCompaniesList`, { Uid: { id }.id }).then((response) => {
            console.log(response.data.data[0])

            if (response.data.success === "true") {
                setsingleCompaniesList(response.data.data[0]);
                setTimeZone(response.data.data[0].TMZ_ID);
                setCOU_ID(response.data.data[0].COU_ID);
                setlang(response.data.data[0].LAN_ID);
                    ...
                    <div className="manage-admins-main-area">
                        <div className="manage-admins-table-area">
                            ...
                            <Form noValidate validated={validated} onSubmit={addSubmit} >
                                ...
                                <Row className="m-3">
                                <input type="hidden" name="comImage" defaultValue={CompIamge} />
                                <input type="hidden" name="updateimage" defaultValue={singleCompaniesList.COM_IMG} />
                                    <Form.Group as={Col} controlId="formGridEmail">
                                        <Form.Label>Company Image</Form.Label>
                                        <Form.Control type="file" placeholder="Company Image"   onChange={changeHandlerImage} />
                                        <img src={{ id }.id ?companyimagepath+singleCompaniesList.COM_IMG : !CompIamge ? Default : CompIamge} alt="preview image" />
                                    </Form.Group>
                                    <input type="hidden" name="urlcombanner" defaultValue={CompanyBanner} />
                                    <input type="hidden" name="updatecompanybanner" defaultValue={singleCompaniesList.COM_BANNER} />
                                    <Form.Group as={Col} controlId="formGridPassword">
                                        <Form.Label>Banner</Form.Label>
                                        <Form.Control type="file" placeholder="Banner" name="comBanner" onChange={changeHandlerBanner} />
                                        <img src={{ id }.id ?bannercompanyimagepath+singleCompaniesList.COM_BANNER : !CompanyBanner ? Default : CompanyBanner} alt="preview image" />
                                    </Form.Group>
